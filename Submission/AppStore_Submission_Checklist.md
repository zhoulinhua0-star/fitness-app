...
## 0. 账号与主体
- [ ] Apple Developer Program 已注册并生效($99/年)。
- [ ] App Store Connect 中已创建 App 记录(Bundle ID:`com.zhoulinhua0-star.FitnessApp2026New`)。
- [ ] Primary Language 设为 English (U.S.);之后可加 Simplified Chinese 本地化。
...
- [x] 权限用途说明已存在(日历、通知)。
      建议后续:把中文用途串本地化出英文版(美区用户会看到)。当前不影响过审,列入待办。
- [ ] `MARKETING_VERSION` = 1.0,`CURRENT_PROJECT_VERSION`(build)= 1;每次上传 build 号要 +1。
- [ ] Archive 一次:Xcode → Product → Destination 选 "Any iOS Device" → Product → Archive。
...
## 3. 隐私政策
- [ ] 把 `Submission/PrivacyPolicy_EN.md`(必要)与 `PrivacyPolicy_ZH.md`(中国区/中文本地化用)托管到一个公开 URL。
      简单做法:GitHub Pages、Notion 公开页、或任意静态托管。
- [ ] 在 App Store Connect 的 App Privacy → Privacy Policy URL 填入该链接。

## 4. App Store 商品页(Product Page)
- [ ] App 名称、副标题(Subtitle)、关键词(Keywords)。
- [ ] 描述(Description)——英文。
- [ ] **截图**:至少 6.9"(iPhone 16 Pro Max 级)与 6.5" 各一组;iPad 若支持也要一组
      (你的工程 `TARGETED_DEVICE_FAMILY = "1,2"` 表示同时支持 iPhone + iPad,所以 **需要 iPad 截图**,
      或考虑改为仅 iPhone 以简化——见待办)。
- [ ] App 图标 1024×1024(无 alpha 通道)。
- [ ] 分类:Primary = Health & Fitness。
- [ ] 年龄分级(Age Rating)问卷:健身内容一般 4+。
- [ ] Support URL(可用一个简单页面或邮箱页)。
...
## 待办(不阻塞美区,但建议尽快)
1. **权限用途串英文本地化** —— 让美区用户看到英文授权说明。可并入整体本地化任务。
2. **iPad 截图 or 降为仅 iPhone** —— 二选一,避免因缺 iPad 截图卡住。
3. **CloudKit 匿名计数器(v1.1)** —— 无登录、最简单方案,美区先上。
4. **中国区:ICP / App 备案** —— 找备案服务商确认当前口径,并行推进。
